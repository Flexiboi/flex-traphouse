...
Traphouses.Functions.ClosePinpad = function() {
    ...
    $.post('https://flex-traphouse/PinpadClose');
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('https://flex-traphouse/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('https://flex-traphouse/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
                Traphouses.Functions.ClosePinpad();
            };
...
$( "#PINcode" ).html(
	"<form action='' method='' name='PINform' id='PINform' autocomplete='off' draggable='true'>" +
		...
		"<br>" +
		"<input type='button' class='PINbutton clear' name='-' value='Wis' id='-' onClick=clearForm(this); />" +
		"<input type='button' class='PINbutton' name='0' value='0' id='0' onClick=addNumber(this); />" +
		"<input type='button' class='PINbutton enter' name='+' value='Probeer' id='+' onClick=submitForm(PINbox); />" +
	"</form>"
);
